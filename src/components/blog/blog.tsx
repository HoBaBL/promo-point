...
const Blog = () => {
    ...
    const [comers, setComers] = useState<ComersType[] | null>([])
        ...
        async function getComers() {
            setLoading(false)
            const { data, error } = await supabase.from("PromoComers").select().in('species', radio).range(0, 50)
            // .eq('species', 'promocode');
            setComers(data);
            setLoading(true)
            if (error !== null) {
                console.log(error)
    ...
    return (
        <div className={style.container}>
            ...
            <p className={style.info}>Промокоды / Все Промокоды</p>
            <h2 className={style.h2}>Новые промокоды</h2>
